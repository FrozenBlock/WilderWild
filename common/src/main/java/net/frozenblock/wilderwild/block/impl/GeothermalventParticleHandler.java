...
public class GeothermalventParticleHandler {
	...
	public static void spawnDormantParticles(Level level, BlockPos pos, GeothermalVentType geothermalVentType, Direction direction, RandomSource random) {
		if (!geothermalVentType.isWater() || random.nextFloat() > DORMANT_BUBBLE_CHANCE) return;
		final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
		final Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, DORMANT_BUBBLE_MIN_VELOCITY, DORMANT_BUBBLE_MAX_VELOCITY);
		level.addParticle(
	...
	public static void spawnActiveParticles(Level level, BlockPos pos, GeothermalVentType geothermalVentType, Direction direction, RandomSource random) {
		if (geothermalVentType.isWater()) {
			if (random.nextFloat() > ACTIVE_BUBBLE_CHANCE) return;
			int count = random.nextInt(MIN_ACTIVE_BUBBLES, MAX_ACTIVE_BUBBLES);
			for (int i = 0; i < count; i++) {
				final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
				Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, ACTIVE_BUBBLE_MIN_VELOCITY_OFFSET, ACTIVE_BUBBLE_MAX_VELOCITY_OFFSET);
				particleVelocity = particleVelocity.add(GeothermalVentBlock.getVelocityFromDistance(pos, direction, particlePos, random, ACTIVE_BUBBLE_RANDOM_VELOCITY));
				level.addParticle(
					ParticleTypes.BUBBLE,
					particlePos.x,
					particlePos.y,
		...
		if (random.nextFloat() <= ACTIVE_DUST_CHANCE) {
			...
			for (int i = 0; i < count; i++) {
				final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
				Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, ACTIVE_DUST_MIN_VELOCITY, ACTIVE_DUST_MAX_VELOCITY);
				particleVelocity = particleVelocity.add(GeothermalVentBlock.getVelocityFromDistance(pos, direction, particlePos, random, ACTIVE_DUST_RANDOM_VELOCITY));
				level.addParticle(
		...
		if (geothermalVentType == GeothermalVentType.LAVA) {
			if (random.nextFloat() <= ACTIVE_LAVA_CHANCE) {
				...
				for (int i = 0; i < count; i++) {
					final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
					Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, ACTIVE_LAVA_MIN_VELOCITY, ACTIVE_LAVA_MAX_VELOCITY);
					particleVelocity = particleVelocity.add(GeothermalVentBlock.getVelocityFromDistance(pos, direction, particlePos, random, ACTIVE_LAVA_RANDOM_VELOCITY));
					level.addParticle(
			...
			if (random.nextFloat() <= ACTIVE_FLAME_CHANCE) {
				final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
				Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, ACTIVE_FLAME_MIN_VELOCITY, ACTIVE_FLAME_MAX_VELOCITY);
				particleVelocity = particleVelocity.add(GeothermalVentBlock.getVelocityFromDistance(pos, direction, particlePos, random, ACTIVE_FLAME_RANDOM_VELOCITY));
	...
	public static void spawnEruptionParticles(Level level, BlockPos pos, GeothermalVentType geothermalVentType, Direction direction, RandomSource random) {
		...
		if (geothermalVentType.isWater()) {
			if (random.nextFloat() <= 0.4F && (!vent || random.nextBoolean())) { // Bubble
				...
				for (int i = 0; i < count; i++) {
					final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
					Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, 1.7D, 4D);
					particleVelocity = particleVelocity.add(GeothermalVentBlock.getVelocityFromDistance(pos, direction, particlePos, random, 0.15D));
					level.addAlwaysVisibleParticle(
						ParticleTypes.BUBBLE,
			...
			if (vent && random.nextFloat() <= 0.3F) { // Smoke
				...
				for (int i = 0; i < count; i++) {
					final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
					Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, 0.05D, 0.15D);
					particleVelocity = particleVelocity.add(GeothermalVentBlock.getVelocityFromDistance(pos, direction, particlePos, random, 0.055D));
					level.addParticle(
						ParticleTypes.LARGE_SMOKE,
						particlePos.x, particlePos.y, particlePos.z,
						particleVelocity.x, particleVelocity.y, particleVelocity.z
			...
			if (random.nextFloat() <= 0.5F) { // Dust Plume
				...
				for (int i = 0; i < count; i++) {
					final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
					Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, 0.8D, 1.4D);
					particleVelocity = particleVelocity.add(GeothermalVentBlock.getVelocityFromDistance(pos, direction, particlePos, random, 0.275D));
					level.addParticle(
						ParticleTypes.DUST_PLUME,
						particlePos.x, particlePos.y, particlePos.z,
						particleVelocity.x, particleVelocity.y, particleVelocity.z
		...
		if (geothermalVentType == GeothermalVentType.LAVA) {
			if (random.nextFloat() <= 0.7F) { // Large Smoke
				...
				for (int i = 0; i < count; i++) {
					final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
					Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, 0.2D, 0.9D);
					particleVelocity = particleVelocity.add(GeothermalVentBlock.getVelocityFromDistance(pos, direction, particlePos, random, 0.275D));
					level.addParticle(
						ParticleTypes.LARGE_SMOKE,
						particlePos.x, particlePos.y, particlePos.z,
			...
			for (int i = 0; i < count; i++) {
				if (random.nextFloat() <= 0.9F) { // Flame
					final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
					Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, 0.1D, 0.6D);
					particleVelocity = particleVelocity.add(GeothermalVentBlock.getVelocityFromDistance(pos, direction, particlePos, random, 0.225D));
					level.addAlwaysVisibleParticle(
						ParticleTypes.FLAME,
						false,
						particlePos.x, particlePos.y, particlePos.z,
			...
			for (int i = 0; i < lavaCount; i++) { // Lava
				...
				final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
				Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, 0.6D, 0.8D);
				particleVelocity = particleVelocity.add(GeothermalVentBlock.getVelocityFromDistance(pos, direction, particlePos, random, 0.2D));
	...
	public static void spawnBaseGeothermalVentParticles(Level level, BlockPos pos, Direction direction, RandomSource random, boolean hydrothermalVent) {
		...
		final int count = hydrothermalVent ? random.nextInt(2, 5) : random.nextInt(0, 3);
		for (int i = 0; i < count; i++) {
			if (random.nextFloat() <= chance) {
				final Vec3 particlePos = GeothermalVentBlock.getParticlePos(pos, direction, random);
				final Vec3 particleVelocity = GeothermalVentBlock.getParticleVelocity(direction, random, 0.001D, 0.005D);
				Particle particle = particleEngine.createParticle(
					hydrothermalVent ? ParticleTypes.LARGE_SMOKE : ParticleTypes.WHITE_SMOKE,
					particlePos.x, particlePos.y, particlePos.z,
					particleVelocity.x, particleVelocity.y, particleVelocity.z
				);
				if (particle != null) {
		...
		if (hydrothermalVent) {
			...
			for (int l = 0; l < VENT_PARTICLE_SPAWN_ATTEMPTS; ++l) {
				...
				if (state.is(Blocks.WATER) && state.getFluidState().isFull()) {
					level.addParticle(
						WWParticleTypes.UNDERWATER_ASH.get(),
