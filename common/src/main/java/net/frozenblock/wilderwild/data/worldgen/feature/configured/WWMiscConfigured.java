...
public final class WWMiscConfigured {
	...
	// MAPLE FOREST
	public static final MapleCollection<FrozenLibConfiguredFeature<BallFeatureConfiguration>> MAPLE_LEAF_LITTER = MapleCollection.NAMES.map(name -> register(name + "_maple_leaf_litter"));
	...
	public static void registerMiscConfigured(BootstrapContext<ConfiguredFeature<?, ?>> entries) {
		...
		MapleCollection.zipApply(MAPLE_LEAF_LITTER, WWBlocks.MAPLE_LEAF_LITTER, (feature, leafLitter) -> {
			...
			feature.makeAndSetHolder(FrozenLibFeatures.BALL,
				new BallFeatureConfiguration(
				...
				new BallBlockPlacement.Builder(BlockStateProvider.simple(WWBlocks.FRAGILE_ICE.get()))
					...
					.outerRingBlockPlacement(
						new BallOuterRingBlockPlacement.Builder(BlockStateProvider.simple(WWBlocks.FRAGILE_ICE.get()))
							...
							.searchingPredicate(BlockPredicate.matchesBlocks(Direction.DOWN.getUnitVec3i(), Blocks.WATER))
							.build()
					).build(),
				Optional.of(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES),
				UniformInt.of(4, 5)
