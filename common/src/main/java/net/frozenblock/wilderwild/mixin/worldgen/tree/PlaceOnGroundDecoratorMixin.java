...
public class PlaceOnGroundDecoratorMixin {
	...
	public boolean wilderWild$onlyPlaceIfLitterTypeIsEnabled(TreeDecorator.Context instance, BlockPos pos, BlockState state) {
		if (state.is(WWBlocks.ACACIA_LEAF_LITTER.get())) return WWWorldgenConfig.ACACIA_LITTER_GENERATION.get();
		if (state.is(WWBlocks.AZALEA_LEAF_LITTER.get())) return WWWorldgenConfig.AZALEA_LITTER_GENERATION.get();
		if (state.is(WWBlocks.BAOBAB_LEAF_LITTER.get())) return WWWorldgenConfig.BAOBAB_LITTER_GENERATION.get();
		if (state.is(WWBlocks.BIRCH_LEAF_LITTER.get())) return WWWorldgenConfig.BIRCH_LITTER_GENERATION.get();
		if (state.is(WWBlocks.CHERRY_LEAF_LITTER.get())) return WWWorldgenConfig.CHERRY_LITTER_GENERATION.get();
		if (state.is(WWBlocks.CYPRESS_LEAF_LITTER.get())) return WWWorldgenConfig.CYPRESS_LITTER_GENERATION.get();
		if (state.is(WWBlocks.DARK_OAK_LEAF_LITTER.get())) return WWWorldgenConfig.DARK_OAK_LITTER_GENERATION.get();
		if (state.is(WWBlocks.JUNGLE_LEAF_LITTER.get())) return WWWorldgenConfig.JUNGLE_LITTER_GENERATION.get();
