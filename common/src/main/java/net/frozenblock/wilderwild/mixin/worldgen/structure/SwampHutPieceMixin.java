...
public class SwampHutPieceMixin {
	...
	public Block wilderWild$newFence(Block original) {
		if (WWWorldgenConfig.NEW_WITCH_HUT_GENERATION.get()) return WWBlocks.WILLOW_FENCE.get();
		return original;
	}
	...
	public Block wilderWild$newLog(Block original) {
		if (WWWorldgenConfig.NEW_WITCH_HUT_GENERATION.get()) return WWBlocks.WILLOW_LOG.get();
		return original;
	}
	...
	public Block wilderWild$newPlanks(Block original) {
		if (WWWorldgenConfig.NEW_WITCH_HUT_GENERATION.get()) return WWBlocks.WILLOW_PLANKS.get();
		return original;
	}
