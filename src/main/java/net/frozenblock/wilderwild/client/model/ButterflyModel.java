...
public class ButterflyModel extends EntityModel<ButterflyRenderState> {
	...
	public void setupAnim(@NotNull ButterflyRenderState renderState) {
		...
		float animation = (renderState.ageInTicks * 0.75F) + (renderState.walkAnimationPos * 0.85F);
		float movementDelta = Math.max(1F - renderState.groundProgress, 0.1F);
		float lengthMultiplier = Math.max(movementDelta * 0.75F, 0.4F);
		float flapLengthMultiplier = lengthMultiplier * (1F - (renderState.downProgress * 0.5F));
		...
		float walkAnimation = renderState.walkAnimationPos * 9F;
