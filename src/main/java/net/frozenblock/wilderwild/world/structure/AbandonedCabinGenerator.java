...
public final class AbandonedCabinGenerator {
    ...
    public static final Holder<StructureTemplatePool> CABIN = Pools.register(
            new StructureTemplatePool(
                    ...
                    ImmutableList.of(
                            Pair.of(ofProcessedSingle(
                                            "abandoned_cabin/cabin/abandoned_cabin_1",
                                            WilderStructureProcessors.ABANDONED_CABIN_PROCESSOR),
                                    1),
                            Pair.of(ofProcessedSingle(
                                            "abandoned_cabin/cabin/abandoned_cabin_2",
                                            WilderStructureProcessors.ABANDONED_CABIN_PROCESSOR),
                                    1),
                            Pair.of(ofProcessedSingle(
                                            "abandoned_cabin/cabin/abandoned_cabin_3",
                                            WilderStructureProcessors.ABANDONED_CABIN_PROCESSOR),
                                    1)
                    ),
    ...
    public static Function<StructureTemplatePool.Projection, SinglePoolElement> ofProcessedSingle(
            final String id,
            final Holder<StructureProcessorList> processorListEntry
    ) {
        return projection -> new SinglePoolElement(
                Either.left(WilderWild.id(id)),
                processorListEntry, projection
        );
    }

    private AbandonedCabinGenerator() {
    }
