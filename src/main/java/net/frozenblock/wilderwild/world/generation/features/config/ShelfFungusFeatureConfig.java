...
public class ShelfFungusFeatureConfig implements FeatureConfiguration {
    public static final Codec<ShelfFungusFeatureConfig> CODEC = RecordCodecBuilder.create(
            (instance) -> instance.group(BuiltInRegistries.BLOCK.byNameCodec().fieldOf("block").flatXmap(ShelfFungusFeatureConfig::validateBlock, DataResult::success).orElse((ShelfFungusBlock) RegisterBlocks.BROWN_SHELF_FUNGUS).forGetter(
                    ...
                    (config) -> config.placeOnWalls), RegistryCodecs.homogeneousList(Registries.BLOCK).fieldOf("can_be_placed_on").forGetter(
