...
public class HeightBasedVineTrunkDecorator extends TreeDecorator {
    public static final Codec<HeightBasedVineTrunkDecorator> CODEC =
            RecordCodecBuilder.create((instance) -> {
                return instance.group(
                                ...
                                Codec.intRange(-63, 319).fieldOf("maxHeight")
                                        .forGetter((treeDecorator) -> {
                                            return treeDecorator.maxHeight;
                                        }),
                                Codec.floatRange(0, 1).fieldOf("vines_count")
                                        .forGetter((treeDecorator) -> {
                ...
                if (pos.getY() <= this.maxHeight) {
                    for (Direction direction : Direction.Plane.HORIZONTAL) {
                        if (abstractRandom.nextFloat() <= this.vines_count) {
                            BlockPos blockPos =
                                    pos.offset(direction.getStepX(), 0,
                                            direction.getStepZ());
