            ...
            for (int z = bz - config.radius; z <= bz + config.radius; z++) {
                for (int y = by - config.radius; y <= by + config.radius; y++) {
                    ...
                    if (distance < radiusSquared) {
                        mutable.set(x, y, z);
                        double sample1 =
                                EasyNoiseSampler.sample(sampler, mutable,
                                        config.multiplier, config.multiplyY,
                                        config.useY);
                        //double sample2 = EasyNoiseSampler.samplePerlinSimplePositive(mutable, config.multiplier, config.multiplyY, config.useY);
                        if (sample1 > config.minThresh &&
                                sample1 < config.maxThresh &&
                                level.getBlockState(mutable)
                                        .is(config.replaceable) &&
                                isWaterNearby(level, mutable, 2)) {
                            generated = true;
                            level.setBlock(mutable,
                                    config.pathBlock.getState(random, mutable),
                                    3);
                        }
                    }
                }
