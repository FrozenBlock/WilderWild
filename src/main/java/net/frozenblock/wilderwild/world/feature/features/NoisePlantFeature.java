        ...
        for (int x = bx - config.radius; x <= bx + config.radius; x++) {
            for (int z = bz - config.radius; z <= bz + config.radius; z++) {
                ...
                if (distance < radiusSquared) {
                    mutable.set(x, world.getHeight(Types.OCEAN_FLOOR, x, z), z);
                    double sample = EasyNoiseSampler.sample(sampler, mutable, config.multiplier, config.multiplyY, config.useY);
                    if (sample > config.minThresh && sample < config.maxThresh && world.getBlockState(mutable).is(config.replaceable) && world.getBlockState(mutable.below()).is(BlockTags.DIRT)) {
                        generated = true;
