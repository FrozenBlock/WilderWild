...
package net.frozenblock.wilderwild.worldgen.structure;
...
public class AbandonedCabinGenerator {
	...
	public static final ResourceKey<StructureTemplatePool> ABANDONED_CABIN = Pools.createKey(WWConstants.string("abandoned_cabin"));

	public static void bootstrapTemplatePool(@NotNull BootstapContext<StructureTemplatePool> pool) {
		...
		context.register(
			...
			new JigsawStructure(
				WWStructures.structure(
					holderGetter.getOrThrow(WWBiomeTags.ABANDONED_CABIN_HAS_STRUCTURE),
	...
	private static @NotNull String string(String name) {
		return WWConstants.string("abandoned_cabin/" + name);
	}
}
