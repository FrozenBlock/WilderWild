...
public final class WWMiscGeneration {
	...
	public static void generateMisc() {
		...
		BiomeModifications.create(WWConstants.id("termite_generation")).add(
			ModificationPhase.ADDITIONS,
			BiomeSelectors.all(),
			(biomeSelectionContext, context) -> {
				if (!WWWorldgenConfig.TERMITE_GENERATION.get()) return;
				if (biomeSelectionContext.hasTag(WWBiomeTags.HAS_TERMITE_MOUND)) {
					context.getGenerationSettings().addFeature(GenerationStep.Decoration.UNDERGROUND_ORES, WWPlacedFeatures.TERMITE_MOUND.getKey());
				}
			});

		BiomeModifications.create(WWConstants.id("surface_decoration")).add(
			ModificationPhase.ADDITIONS,
			BiomeSelectors.all(),
			(biomeSelectionContext, context) -> {
				final BiomeModificationContext.GenerationSettingsContext generationSettings = context.getGenerationSettings();

				if (WWWorldgenConfig.COARSE_DIRT_DECORATION.get()) {
				...
				if (WWWorldgenConfig.MUD_DECORATION.get()) {
				...
				if (WWWorldgenConfig.PACKED_MUD_DECORATION.get()) {
				...
				if (WWWorldgenConfig.GRAVEL_DECORATION.get()) {
				...
				if (WWWorldgenConfig.STONE_DECORATION.get()) {
				...
				if (WWWorldgenConfig.MOSS_DECORATION.get()) {
				...
				if (WWWorldgenConfig.AUBURN_MOSS_DECORATION.get()) {
				...
				if (WWWorldgenConfig.PALE_MOSS_DECORATION.get()) {
				...
				if (WWWorldgenConfig.SCORCHED_SAND_DECORATION.get()) {
				...
				if (WWWorldgenConfig.SCORCHED_RED_SAND_DECORATION.get()) {
				...
				if (WWWorldgenConfig.SANDSTONE_DECORATION.get()) {
				...
				if (WWWorldgenConfig.CLAY_DECORATION.get()) {
				...
				if (WWWorldgenConfig.CLEARING_DECORATION.get()) {
				...
				if (WWWorldgenConfig.TAIGA_BOULDER_DECORATION.get()) {
				...
				if (WWWorldgenConfig.SNOW_PILE_DECORATION.get()) {
				...
				if (WWWorldgenConfig.BASIN_DECORATION.get()) {
		...
		BiomeModifications.create(WWConstants.id("surface_transitions")).add(
			...
			(biomeSelectionContext, context) -> {
				...
				if (WWWorldgenConfig.SAND_TRANSITION_GENERATION.get()) {
				...
				if (WWWorldgenConfig.RED_SAND_TRANSITION_GENERATION.get()) {
				...
				if (WWWorldgenConfig.COARSE_DIRT_TRANSITION_GENERATION.get()) {
				...
				if (WWWorldgenConfig.GRAVEL_TRANSITION_GENERATION.get()) {
				...
				if (WWWorldgenConfig.MUD_TRANSITION_GENERATION.get()) {
				...
				if (WWWorldgenConfig.STONE_TRANSITION_GENERATION.get()) {
		...
		BiomeModifications.create(WWConstants.id("river_pools")).add(
			ModificationPhase.ADDITIONS,
			BiomeSelectors.all(),
			(biomeSelectionContext, context) -> {
