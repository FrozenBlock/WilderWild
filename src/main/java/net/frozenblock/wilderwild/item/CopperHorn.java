...
public class CopperHorn extends InstrumentItem {
    ...
    public static ItemStack getStackForInstrument(Item item,
    ...
    private static void setInstrument(ItemStack stack,
                                      Holder<Instrument> instrument) {
        CompoundTag nbtCompound = stack.getOrCreateTag();
        nbtCompound.putString(
                INSTRUMENT_KEY, (instrument.unwrapKey().orElseThrow(
                        () -> new IllegalStateException(
                                "Invalid instrument"))).location().toString()
        );
    }

    @Override
    public void fillItemCategory(@NotNull CreativeModeTab group,
                                 @NotNull NonNullList<ItemStack> stacks) {
        if (this.allowedIn(group)) {
            for (Holder<Instrument> registryEntry : Registry.INSTRUMENT.getTagOrEmpty(
                    this.instrumentTag)) {
                stacks.add(getStackForInstrument(RegisterItems.COPPER_HORN,
                        registryEntry));
            }
        }
    ...
    public Optional<Holder<Instrument>> getInstrument(ItemStack stack) {
        ...
        if (nbtCompound != null) {
            ResourceLocation identifier = ResourceLocation.tryParse(
                    nbtCompound.getString(INSTRUMENT_KEY));
            if (identifier != null) {
    ...
    private static void playSound(Instrument instrument, Player user,
                                  Level level,
                                  Holder<Instrument> instrumentHolder) {
        SoundEvent soundEvent = instrument.soundEvent();
        float range = instrument.range() / 16;
        int note = (int) ((-user.getXRot() + 90) / 7.5);

        if (!level.isClientSide) {
            float soundPitch = !user.isShiftKeyDown() ?
                    (float) Math.pow(2, (note - 12) / 12) :
                    (float) Math.pow(2, 0.01111F * -user.getXRot());
            ...
            FrozenSoundPackets.createMovingRestrictionLoopingSound(level, user,
                    soundEvent, SoundSource.RECORDS, range, soundPitch,
                    WilderWild.id("instrument"));
        }
        level.gameEvent(GameEvent.INSTRUMENT_PLAY, user.position(),
