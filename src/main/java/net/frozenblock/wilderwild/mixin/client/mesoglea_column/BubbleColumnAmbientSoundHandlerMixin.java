...
package net.frozenblock.wilderwild.mixin.client.mesoglea_column;
...
public class BubbleColumnAmbientSoundHandlerMixin {
	...
	private static boolean wilderWild$filterWithMesoglea(BlockState state, Operation<Boolean> operation) {
		return operation.call(state) || MesogleaBlock.hasBubbleColumn(state);
	}
	...
	private static boolean wilderWild$mesogleaAndBubbleCheck(BlockState state, Operation<Boolean> operation) {
		return operation.call(state) || MesogleaBlock.hasBubbleColumn(state);
	}
	...
	private static boolean wilderWild$blockStateValueCheckWithMesoglea(BlockState state, Operation<Boolean> operation) {
		return state.hasProperty(BubbleColumnBlock.DRAG_DOWN) ? operation.call(state) : MesogleaBlock.isDraggingDown(state);
	}
