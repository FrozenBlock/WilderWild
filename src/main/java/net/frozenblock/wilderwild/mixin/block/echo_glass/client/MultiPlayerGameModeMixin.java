...
public abstract class MultiPlayerGameModeMixin {
	...
	public boolean wilderWild$destroyBlockEchoB(
		...
		if (state.getBlock() instanceof EchoGlassBlock && EchoGlassBlock.canDamage(state) && !this.getPlayerMode().isCreative()) {
			if (EnchantmentHelper.getItemEnchantmentLevel(Enchantments.SILK_TOUCH, this.minecraft.player.getMainHandItem()) < 1) {
				EchoGlassBlock.setDamagedState(instance, pos, state);
			}
			return true;
		}
