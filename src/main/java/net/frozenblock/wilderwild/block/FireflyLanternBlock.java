            ...
            if (lantern.invEmpty()) {
                if (stack.getItem() instanceof FireflyBottle bottle) {
                    if (lantern.getFireflies().size() < 4) {
                        ...
                        world.playSound(null, pos, RegisterSounds.ITEM_BOTTLE_PUT_IN_LANTERN_FIREFLY, SoundCategory.BLOCKS, 1.0F, world.random.nextFloat() * 0.2f + 0.8f);
                        lantern.updateSync();
                        return ActionResult.SUCCESS;
                    }
                }
                if (stack.isOf(Items.GLASS_BOTTLE)) {
                    if (!lantern.getFireflies().isEmpty()) {
                        ...
                        world.playSound(null, pos, RegisterSounds.ITEM_BOTTLE_CATCH_FIREFLY, SoundCategory.BLOCKS, 1.0F, world.random.nextFloat() * 0.2f + 0.8f);
                        if (!player.isCreative()) {
                            player.getStackInHand(hand).decrement(1);
                        }
