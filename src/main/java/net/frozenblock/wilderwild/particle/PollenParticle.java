...
public class PollenParticle extends TextureSheetParticle {
    ...
    PollenParticle(ClientLevel level, SpriteSet spriteProvider, double x,
        ...
        this.lifetime = (int) (16 / (Math.random() * 0.8D + 0.2D));
    ...
    public void tick() {
        super.tick();
        if (!this.alreadyBoosted && this.age > this.lifetime / 5 &&
                !this.onGround && this.hasCarryingWind) {
    ...
    @Environment(EnvType.CLIENT)
    public static class PollenFactory
        ...
        public Particle createParticle(SimpleParticleType defaultParticleType,
                                       ...
                                       double y, double z, double g, double h,
                                       double i) {
            PollenParticle pollenParticle =
                    new PollenParticle(clientLevel, this.spriteProvider, x, y,
                            z, 0, -0.800000011920929D, 0) {
                    };
            pollenParticle.lifetime =
                    Mth.randomBetweenInclusive(clientLevel.random, 500, 1000);
    ...
    @Environment(EnvType.CLIENT)
    public static class DandelionFactory
            implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet spriteProvider;
        ...
        public Particle createParticle(@NotNull SimpleParticleType type,
                                       ClientLevel level, double x, double y,
                                       double z, double xSpeed, double ySpeed,
                                       double zSpeed) {
            double windex =
                    Math.cos((level.getDayTime() * Math.PI) / 12000) * 1.1;
            double windZ =
                    -Math.sin((level.getDayTime() * Math.PI) / 12000) * 1.1;
            PollenParticle pollenParticle =
                    new PollenParticle(level, this.spriteProvider, x, y, z,
                            windex, -0.800000011920929D, windZ);
            pollenParticle.lifetime =
                    Mth.randomBetweenInclusive(level.random, 500, 1000);
            pollenParticle.gravity = 0.01F;
    ...
    @Environment(EnvType.CLIENT)
    public static class MilkweedFactory
            implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet spriteProvider;
    ...
    @Environment(EnvType.CLIENT)
    public static class ControlledDandelionFactory
            implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet spriteProvider;
        ...
        public Particle createParticle(SimpleParticleType defaultParticleType,
            ...
            double windZ = i * 1.1;
            PollenParticle pollenParticle =
                    new PollenParticle(clientLevel, this.spriteProvider, d, e,
                            f, windex, (h / 2) - 0.800000011920929D, windZ);
            pollenParticle.lifetime =
                    Mth.randomBetweenInclusive(clientLevel.random, 500, 1000);
            pollenParticle.gravity = 0.01F;
            pollenParticle.xd =
                    (windex + clientLevel.random.triangle(0, 0.8)) / 17;
            pollenParticle.zd =
                    (windZ + clientLevel.random.triangle(0, 0.8)) / 17;
            pollenParticle.setColor(250F / 255F, 250F / 255F, 250F / 255F);
            pollenParticle.hasCarryingWind = true;
            return pollenParticle;
        }
    ...
    @Environment(EnvType.CLIENT)
    public static class ControlledMilkweedFactory
            implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet spriteProvider;
        ...
        public Particle createParticle(SimpleParticleType defaultParticleType,
            ...
            pollenParticle.gravity = 0.016F;
            pollenParticle.xd =
                    (windex + clientLevel.random.triangle(0, 0.8)) / 20;
            pollenParticle.zd =
                    (windZ + clientLevel.random.triangle(0, 0.8)) / 20;
            pollenParticle.setColor(250F / 255F, 250F / 255F, 250F / 255F);
            pollenParticle.hasCarryingWind = true;
            return pollenParticle;
        }
