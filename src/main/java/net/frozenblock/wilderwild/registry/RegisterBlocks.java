...
public abstract class RegisterBlocks {
    ...
    public static final Block HANGING_TENDRIL = registerBlock("hanging_tendril",
            new HangingTendrilBlock(FabricBlockSettings.copyOf(Blocks.SCULK_SENSOR).collidable(false).luminance((state) -> 1).emissiveLighting((state, world, pos) -> SculkSensorBlock.getPhase(state) == SculkSensorPhase.ACTIVE), 8), ItemGroup.REDSTONE);
