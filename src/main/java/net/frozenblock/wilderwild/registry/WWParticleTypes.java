...
public final class WWParticleTypes {
	...
	public static final ParticleType<FloatingSculkBubbleParticleOptions> FLOATING_SCULK_BUBBLE = register(
		"floating_sculk_bubble", false, FloatingSculkBubbleParticleOptions.DESERIALIZER, particleType -> FloatingSculkBubbleParticleOptions.CODEC
	);
	public static final ParticleType<WindParticleOptions> WIND = register(
		"wind", false, WindParticleOptions.DESERIALIZER, particleType -> WindParticleOptions.CODEC
	);
	...
	public static final ParticleType<LeafClusterParticleOptions> LEAF_CLUSTER_SPAWNER = register(
		"leaf_cluster", false, LeafClusterParticleOptions.DESERIALIZER, leafClusterParticleOptionsParticleType -> LeafClusterParticleOptions.CODEC
	);
	...
	private static @NotNull ParticleType<LeafParticleOptions> createLeafParticle(ResourceLocation location) {
		return register(
			location, false, LeafParticleOptions.DESERIALIZER, particleType -> LeafParticleOptions.CODEC
		);
	}

	private static @NotNull ParticleType<LeafParticleOptions> createLeafParticle(
		ResourceLocation location, Block sourceBlock, float particleChance, Supplier<Double> frequencyModifier, float quadSize, float particleGravityScale
	) {
	...
	private static @NotNull ParticleType<LeafParticleOptions> createLeafParticle(
		ResourceLocation location, Block sourceBlock, Supplier<Double> frequencyModifier
	) {
		return createLeafParticle(location, sourceBlock, 0.0225F, frequencyModifier, 0.125F, 3F);
	}
	...
	private static <T extends ParticleOptions> ParticleType<T> register(@NotNull String name, boolean alwaysShow, @NotNull ParticleOptions.Deserializer<T> factory, Function<ParticleType<T>, Codec<T>> codecGetter) {
		return Registry.register(BuiltInRegistries.PARTICLE_TYPE, WWConstants.id(name), new ParticleType<T>(alwaysShow, factory) {
			@Override
	...
	@NotNull
	private static <T extends ParticleOptions> ParticleType<T> register(@NotNull ResourceLocation location, boolean alwaysShow, @NotNull ParticleOptions.Deserializer<T> factory, Function<ParticleType<T>, Codec<T>> codecGetter) {
		return Registry.register(BuiltInRegistries.PARTICLE_TYPE, location, new ParticleType<T>(alwaysShow, factory) {
			@Override
			public Codec<T> codec() {
				return codecGetter.apply(this);
			}
