...
public class RegisterStructures {
    ...
    public static final ResourceKey<StructureSet> ABANDONED_CABINS_KEY =
            ofSet("abandoned_cabins");

    private static ResourceKey<StructureSet> ofSet(String id) {
        return ResourceKey.create(Registry.STRUCTURE_SET_REGISTRY,
                WilderWild.id(id));
    }

    public static Holder<StructureSet> initAndGetDefault(
            Registry<StructureSet> registry) {
        return registry.holders().iterator().next();
    }

    private static Holder<StructureSet> register(ResourceKey<StructureSet> key,
                                                 StructureSet structureSet) {
        return BuiltinRegistries.register(BuiltinRegistries.STRUCTURE_SETS, key,
                structureSet);
    }
    ...
    private static final ResourceKey<Structure> ABANDONED_CABIN_KEY =
            of("abandoned_cabin");
    ...
    public static final Holder<Structure> ABANDONED_CABIN = register(
            ...
            new JigsawStructure(
                    ...
                    5,
                    UniformHeight.of(VerticalAnchor.absolute(-40),
    ...
    public static final Holder<StructureSet> ABANDONED_CABINS = register(
            ABANDONED_CABINS_KEY, ABANDONED_CABIN,
            new RandomSpreadStructurePlacement(11, 3, RandomSpreadType.LINEAR,
                    20388232)
    );

    public static void init() {
        WilderWild.logWild("Registering Structures for",
    ...
    private static ResourceKey<Structure> of(String id) {
        return ResourceKey.create(Registry.STRUCTURE_REGISTRY,
                WilderWild.id(id));
    }
    ...
    private static Structure.StructureSettings createConfig(
        ...
        return new Structure.StructureSettings(getOrCreateBiomeTag(biomeTag),
                spawns, featureStep, terrainAdaptation);
    }

    private static Structure.StructureSettings createConfig(
            TagKey<Biome> biomeTag, GenerationStep.Decoration featureStep,
            TerrainAdjustment terrainAdaptation) {
        return createConfig(biomeTag, Map.of(), featureStep, terrainAdaptation);
    }

    private static Structure.StructureSettings createConfig(
            TagKey<Biome> biomeTag, TerrainAdjustment terrainAdaptation) {
        return createConfig(biomeTag, Map.of(),
                GenerationStep.Decoration.SURFACE_STRUCTURES,
                terrainAdaptation);
    }

    private static Holder<Structure> register(ResourceKey<Structure> key,
                                              Structure structure) {
        return BuiltinRegistries.register(BuiltinRegistries.STRUCTURES, key,
                structure);
    }
