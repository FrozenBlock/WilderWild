...
public class AncientHornProjectileRenderer extends EntityRenderer<AncientHornProjectile> {
    ...
    private static final RenderLayer LAYER = RenderLayer.getEntityTranslucentCull(TEXTURE);
    private static final RenderLayer OVERLAY = RenderLayer.getEntityTranslucentEmissive(TEXTURE);
    ...
    public void render(AncientHornProjectile projectile, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        ...
        vertexConsumers.getBuffer(OVERLAY);

                //ItemRenderer.getDirectItemGlintConsumer(vertexConsumers, this.model.getLayer(this.getTexture(projectile)), false, false);
