...
public class DisplayLanternBlockEntityRenderer<T extends DisplayLanternBlockEntity> implements BlockEntityRenderer<T> {
	...
	@Override
	public void render(@NotNull T lantern, float tickDelta, @NotNull PoseStack matrices, @NotNull MultiBufferSource vertexConsumers, int light, int overlay) {
		...
		if (!lantern.invEmpty() && stack.isPresent()) {
			...
			matrices.mulPose(Axis.YP.rotation((lantern.age + tickDelta) / 20F));
			...
			for (DisplayLanternBlockEntity.FireflyInLantern entity : lantern.getFireflies()) {
				double ageDelta = entity.age + tickDelta;
				FireflyRenderer.renderFirefly(matrices, vertexConsumers, light, entity.getCustomName().toLowerCase().contains("nectar"), overlay, entity.age, tickDelta, entity.flickers, entity.getColor(), 1F, (float) entity.pos.x, lantern.clientHanging ? 0.38F : 0.225F + (float) Math.sin(ageDelta * 0.03F) * 0.15F, (float) entity.pos.z, Minecraft.getInstance().gameRenderer.getMainCamera().rotation());
			}
		}
	}
