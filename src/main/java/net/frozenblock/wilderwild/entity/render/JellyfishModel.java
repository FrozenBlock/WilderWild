...
public class JellyfishModel<T extends Jellyfish> extends HierarchicalModel<T> {
    ...
    public void setupAnim(@NotNull T jellyfish, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        this.body.xScale = limbSwingAmount == 0 ? Mth.lerp(ageInTicks, squashStretch, sinIdle) : Mth.lerp(ageInTicks, sinIdle, squashStretch);
        this.body.zScale = limbSwingAmount == 0 ? Mth.lerp(ageInTicks, squashStretch, sinIdle) : Mth.lerp(ageInTicks, sinIdle, squashStretch);
