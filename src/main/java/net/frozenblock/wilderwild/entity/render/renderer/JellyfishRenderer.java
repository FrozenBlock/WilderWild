...
public class JellyfishRenderer extends MobRenderer<Jellyfish, JellyfishModel<Jellyfish>> {
	...
	@Override
	public void setupRotations(@NotNull Jellyfish jelly, @NotNull PoseStack poseStack, float ageInTicks, float bodyYaw, float tickDelta) {
		super.setupRotations(jelly, poseStack, ageInTicks, bodyYaw, tickDelta);

		float scale = jelly.getScale();
		poseStack.translate(0F, -1F * scale, 0F);
		poseStack.scale(0.8F, 0.8F, 0.8F);
		float jellyScale = (jelly.prevScale + tickDelta * (jelly.scale - jelly.prevScale)) * scale;
