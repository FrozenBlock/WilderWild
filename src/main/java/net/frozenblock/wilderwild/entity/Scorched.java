...
public class Scorched extends Spider {
	...
	public static boolean checkScorchedSpawnRules(
		EntityType<? extends Scorched> type, @NotNull ServerLevelAccessor level, MobSpawnType spawnType, BlockPos pos, RandomSource random
	) {
		if (level.getDifficulty() == Difficulty.PEACEFUL) return false;
		if (!MobSpawnType.isSpawner(spawnType) && !WWEntityConfig.get().scorched.spawnScorched) return false;
		return MobSpawnType.ignoresLightRequirements(spawnType) || Scorched.isDarkEnoughToSpawn(level, pos, random);
	}
