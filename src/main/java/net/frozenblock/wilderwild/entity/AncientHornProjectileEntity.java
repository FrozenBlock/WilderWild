...
public class AncientHornProjectileEntity extends PersistentProjectileEntity {
    ...
    public boolean isNoClip() {
        ...
        if (hitResult.getType() == HitResult.Type.BLOCK) {
            ...
            if (state.isIn(this.NON_COLLIDE) || nonSolidAndNotSculk(world, ((BlockHitResult)hitResult).getBlockPos())) {
                return true;
            }
        } return false;
    }
    public static boolean nonSolidAndNotSculk(World world, BlockPos pos) {
        ...
        if (state.isOf(Blocks.SCULK_SENSOR) || state.isOf(Blocks.SCULK_SHRIEKER) || state.isOf(RegisterBlocks.SCULK_ECHOER) || state.isAir()) { return false; }
