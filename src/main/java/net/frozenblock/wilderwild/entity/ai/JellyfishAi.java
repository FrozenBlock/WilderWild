...
public class JellyfishAi {
    ...
    private static final ImmutableList<SensorType<? extends Sensor<? super Jellyfish>>>
    ...
    public static Brain<Jellyfish> makeBrain(Jellyfish jellyfish,
                                             Dynamic<?> dynamic) {
        Brain.Provider<Jellyfish> provider =
                Brain.provider(MEMORY_TYPES, SENSOR_TYPES);
    ...
    private static void initIdleActivity(Brain<Jellyfish> brain) {
        brain.addActivity(
                Activity.IDLE,
                ImmutableList.of(
                        Pair.of(3, new StartAttacking<>(
                                JellyfishAi::findNearestValidAttackTarget)),
                        ...
                        Pair.of(
                                ...
                                new GateBehavior<>(
                                        ...
                                        ImmutableList.of(
                                                Pair.of(new RandomSwim(0.5F),
                                                        2),
                                                Pair.of(new RandomStroll(0.15F,
                                                        false), 2),
                                                Pair.of(new RunIf<>(
    ...
    private static void initFightActivity(Jellyfish jellyfish,
                                          Brain<Jellyfish> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(
                Activity.FIGHT,
                10,
                ImmutableList.of(
                        new StopAttackingIfTargetInvalid<>(
                                livingEntity -> !jellyfish.canTargetEntity(
                                        livingEntity),
                                JellyfishAi::onTargetInvalid, false
                        ),
                        new SetEntityLookTarget(
                                livingEntity -> isTarget(jellyfish,
                                        livingEntity),
                                (float) jellyfish.getAttributeValue(
                                        Attributes.FOLLOW_RANGE)),
                        new SetWalkTargetFromAttackTargetIfTargetOutOfReach(
                                JellyfishAi::getSpeedModifierChasing)
                ),
                MemoryModuleType.ATTACK_TARGET
        );
    }

    private static boolean isTarget(Jellyfish jellyfish,
                                    LivingEntity livingEntity) {
        return jellyfish.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET)
                .filter(livingEntity2 -> livingEntity2 == livingEntity)
                .isPresent();
    }
    ...
    public static void updateActivity(Jellyfish jellyfish) {
        ...
        if (activity == Activity.FIGHT &&
                brain.getActiveNonCoreActivity().orElse(null) !=
                        Activity.FIGHT) {
            brain.setMemoryWithExpiry(MemoryModuleType.HAS_HUNTING_COOLDOWN,
                    true, 2400L);
        }
    ...
    private static void onTargetInvalid(Jellyfish jellyfish,
                                        LivingEntity livingEntity) {
    ...
    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(
            Jellyfish jellyfish) {
        return jellyfish.getBrain()
