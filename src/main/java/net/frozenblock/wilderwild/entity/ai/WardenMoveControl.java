        ...
        if (this.isEntityTouchingWaterOrLava(this.entity)) {
            if (this.buoyant) {
                if ((this.entity.getBrain().hasMemoryValue(MemoryModuleType.ROAR_TARGET) || this.entity.getBrain().hasMemoryValue(MemoryModuleType.ATTACK_TARGET))) {
                    if (this.entity.getBrain().getMemory(MemoryModuleType.ROAR_TARGET).isPresent()) {
                        if (this.entity.getBrain().getMemory(MemoryModuleType.ROAR_TARGET).get().getY() > this.entity.getY()) {
                            this.entity.setDeltaMovement(this.entity.getDeltaMovement().add(0.0D, 0.008D, 0.0D));
                        } else {
                            this.entity.setDeltaMovement(this.entity.getDeltaMovement().add(0.0D, -0.008D, 0.0D));
                        }

                    } else if (this.entity.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).isPresent()) {
                        if (this.entity.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).get().getY() > this.entity.getY()) {
                            this.entity.setDeltaMovement(this.entity.getDeltaMovement().add(0.0D, 0.008D, 0.0D));
                        } else {
                            this.entity.setDeltaMovement(this.entity.getDeltaMovement().add(0.0D, -0.008D, 0.0D));
                        }
